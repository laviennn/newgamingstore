...
export function DashboardHistoryClient({ mergedHistory }: { mergedHistory: any[] }) {
  ...
  return (
    <div className="bg-[#121212] border border-white/5 rounded-2xl p-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
        <h3 className="text-lg font-bold text-white">Riwayat Transaksi Terbaru</h3>
        
        {/* Limit Dropdown */}
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-gray-400 tracking-widest uppercase">Tampilkan:</span>
          <div className="relative">
            <select 
              ...
              className="bg-transparent border border-white/10 rounded-2xl px-4 py-2 text-sm text-gray-300 focus:outline-none focus:border-blue-500 appearance-none transition-colors pr-10"
            >
          ...
          {mergedHistory.length > 0 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-white/5 pt-4 mt-4">
              <div className="text-xs text-gray-400 font-medium">
                Menampilkan <span className="text-white font-bold">{startIndex}</span> - <span className="text-white font-bold">{endIndex}</span> dari <span className="text-white font-bold">{mergedHistory.length}</span> data
              </div>
