  ...
  try {
    if (id) {
      const { error } = await supabase
        ...
        .update({ category, name, logo_url, qr_image_url, account_number, account_name, is_active })
        .eq("id", id);
      if (error) throw error;
    } else {
      const { error } = await supabase
        .from("payment_channels")
        .insert([{ category, name, logo_url, qr_image_url, account_number, account_name, is_active }]);
      if (error) throw error;
    }
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === "23503" || error.message?.includes("foreign key constraint") || error.message?.includes("violates")) {
        // Automatically deactivate instead of hard delete to preserve historical order logs
        await supabase.from("payment_channels").update({ is_active: false }).eq("id", id);
        revalidatePath("/admin/payments");
        revalidatePath("/");
        return { 
          success: true, 
