...
export function Footer({ domain, themeConfig, paymentChannels }: { domain: string, themeConfig: any, paymentChannels: any[] }) {
  ...
  const displayDesc = seoDescription || "Platform top up game terlengkap, cepat & termurah di Indonesia dengan layanan 24/7.";
      ...
      <div className={`container mx-auto px-4 pt-16 md:pt-24 pb-12`}>
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-8">
          ...
          <div className="md:col-span-4 space-y-6">
            ...
            <div className="pt-4">
              <h3 className="text-sm font-semibold text-foreground mb-4">Pembayaran Aman</h3>
              <div className="flex flex-wrap gap-2">
                ...
                {paymentChannels.length > 5 && (
                  <div className="h-8 flex items-center justify-center px-2 text-xs text-primary font-semibold">
                    +{paymentChannels.length - 5} lainnya
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="md:col-span-3 space-y-6">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-semibold text-foreground">Chat WhatsApp</span>
              </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3 text-sm text-slate-400">
                ...
                <span>Jam Operasional: {operationalHours || "08:00 - 23:00 WIB"}</span>
              </div>
              {email && (
                <div className="flex items-center gap-3 text-sm text-slate-400">
            ...
            <div className="pt-4 space-y-2">
              <h3 className="text-sm font-semibold text-foreground">Jaminan Transaksi</h3>
              <p className="text-xs text-slate-400 leading-relaxed">Garansi uang kembali 100% apabila layanan gagal terkirim karena kesalahan sistem.</p>
              <div className="flex items-center gap-1.5 text-xs font-bold text-emerald-400 mt-2">
                <ShieldCheck className="w-4 h-4" /> 100% Legal & Aman
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <ul className="space-y-3 text-sm text-slate-400">
              <li><Link href="/track" className="hover:text-primary transition-colors">Cek Pesanan</Link></li>
              <li><Link href="/login" className="hover:text-primary transition-colors">Dashboard</Link></li>
              <li><Link href="/prices" className="hover:text-primary transition-colors">Daftar Harga</Link></li>
          ...
          <div className="md:col-span-3 space-y-6 flex flex-col items-start md:items-end text-left md:text-right">
            <div className="w-full">
               <h3 className="text-sm font-semibold text-foreground md:text-right mb-6">Sosial Media</h3>
      ...
      <div className="border-t border-border bg-background">
        <div className="container mx-auto px-4 py-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-xs text-slate-500">
            Copyright © {new Date().getFullYear()} <span className="font-bold text-muted-foreground">{displayTitle}</span>. All Rights Reserved.
          </p>
          
          <div className="flex items-center gap-6 text-xs font-semibold">
            <Link href="/terms" className="text-primary hover:text-primary/80">Syarat & Ketentuan</Link>
            <Link href="/privacy" className="text-primary hover:text-primary/80">Kebijakan Privasi</Link>
            
            <div className="bg-card text-foreground border border-border px-3 py-1.5 rounded-full flex items-center gap-2 cursor-pointer hover:bg-card/80">
              <div className="w-4 h-4 rounded-full bg-white overflow-hidden flex items-center justify-center relative">
                 <div className="w-full h-1/2 bg-red-600 top-0 absolute" />
              </div>
