...
import { Info, ShoppingCart, ChevronDown, ChevronUp, Ticket, CheckCircle2, Loader2, AlertTriangle, ArrowUpDown, ArrowDownWideNarrow, ArrowUpWideNarrow } from "lucide-react";
...
import { formatCurrency, getProductPrice, getProductName, isProductAvailableInCurrency, sortProductsByCurrencyPrice, getPhoneConfigForCurrency, type Currency } from "@/lib/currencyUtils";
...
export function StorefrontGameForm({
  ...
  // Sort state: default to 'asc' (lowest price / termurah first)
  ...
  // Sort available products according to selected currency and sort order
  const sortedAvailableProducts = React.useMemo(() => {
    return sortProductsByCurrencyPrice(availableProducts, currency, priceSortOrder);
  }, [availableProducts, currency, priceSortOrder]);
    ...
    <form onSubmit={handleCheckout} className="space-y-6 lg:mt-6 w-full relative pb-36 md:pb-0">
      ...
      <div className="border border-border/40 shadow-xl overflow-hidden rounded-xl bg-theme-card">
        <div className="border-b border-border/30 bg-theme-background p-4 relative">
          ...
          <div className="flex items-center gap-4 pl-3">
            <NumberBadge num={2} />
            <h2 className="text-lg font-bold text-white tracking-wide">{dict.game_select_nominal || "Pilih Nominal Top Up"}</h2>
          </div>
        </div>
        ...
        <div className="p-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
            ...
            {hasVariants ? (
              <div className="flex flex-wrap items-center gap-1.5 bg-theme-background p-1.5 rounded-xl border border-border/20">
                {variants.map((v) => (
                  <button
                    key={v}
                    type="button"
            ...
            {/* Price Sort Controls */}
            <div className="flex items-center gap-1 bg-theme-background p-1 rounded-xl border border-border/20 w-fit self-end sm:self-auto">
              <span className="px-2.5 py-1 text-xs font-semibold text-gray-400 hidden sm:inline-flex items-center gap-1.5">
                <ArrowUpDown className="w-3.5 h-3.5 text-theme-primary" />
                <span>{dict.game_sort_label || 'Urutan:'}</span>
              </span>
              <button
                type="button"
                ...
                title={dict.game_sort_price_asc || 'Paling Murah'}
              >
                <ArrowDownWideNarrow className="w-3.5 h-3.5" />
                <span>{dict.game_sort_price_asc || 'Termurah'}</span>
              </button>
              <button
                type="button"
                ...
                title={dict.game_sort_price_desc || 'Paling Mahal'}
              >
                <ArrowUpWideNarrow className="w-3.5 h-3.5" />
                <span>{dict.game_sort_price_desc || 'Tertinggi'}</span>
              </button>
            </div>
          </div>
          ...
          {(() => {
            const displayProducts = hasVariants && selectedVariant
              ? sortedAvailableProducts.filter((p: any) => p.variant_type === selectedVariant)
