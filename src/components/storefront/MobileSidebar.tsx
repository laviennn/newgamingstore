...
export function MobileSidebar({
  ...
  const isLoggedIn = !!user || isUsernameMember;

  useEffect(() => {
    if (!user?.email || isUsernameMember) return;
  ...
  const subtitle = isUsernameMember
    ? `@${memberSession!.username}`
    : user?.email || '';

  const nameForInitials = isUsernameMember
    ? memberSession!.username
    : user?.user_metadata?.name || 'User';
    ...
    <Sheet>
      ...
      <SheetContent
        ...
        <div className='p-4 text-center border-t border-border/50'>
          <p className='text-xs text-muted-foreground'>
            &copy; 2026 {process.env.NEXT_PUBLIC_APP_NAME || 'Gaming Store'}.
            All rights reserved.
          </p>
        </div>
      </SheetContent>
