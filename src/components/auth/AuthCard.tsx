  ...
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (val.length > 0) {
      if (!/^(08|62|\+62)/.test(val)) {
        setPhoneError(dict.auth_phone_err_format);
      } else if (val.length < 10 || val.length > 15) {
        setPhoneError(dict.auth_phone_err_length);
      } else {
        setPhoneError('');
      }
    } else {
        ...
        <div className='hidden md:flex md:w-1/2 relative'>
          ...
          <div className='absolute bottom-10 left-10 right-10'>
            <h1 className='text-white text-5xl font-black mb-2 tracking-tight'>
              {dict.auth_banner_title}
            </h1>
            <p className='text-gray-300 text-sm'>
              {dict.auth_banner_desc}
            </p>
          </div>
        </div>
        ...
        <div className='w-full md:w-1/2 p-8 md:p-12 relative flex flex-col justify-center overflow-y-auto'>
          ...
          <div className='mb-8'>
            <h2 className='text-3xl font-bold text-white mb-2'>
              {mode === 'login' ? dict.auth_login_title : dict.auth_reg_title}
            </h2>
            <p className='text-gray-400 text-sm'>
              {mode === 'login'
                ? isUsernameMode
                  ? dict.auth_login_user_sub
                  : dict.auth_login_subtitle
                : dict.auth_reg_subtitle}
            </p>
          </div>
          ...
          <form
            ...
            <div className='space-y-1.5 relative'>
              <label className='text-sm font-medium text-gray-300'>
                {dict.auth_label_password}
              </label>
              <div className='relative'>
                <input
                  name='password'
                  type={showPassword ? 'text' : 'password'}
                  placeholder={dict.auth_ph_password}
                  required
                  autoComplete={
                    isUsernameMode
                      ? 'current-password'
              ...
              <div className='flex items-center justify-between'>
                <label className='flex items-center gap-2 cursor-pointer group'>
                  ...
                  <span className='text-sm text-gray-400 group-hover:text-white transition-colors'>
                    {dict.auth_remember_me}
                  </span>
                </label>
                <Link
                  href='#'
                  className='text-sm text-blue-500 hover:text-blue-400 transition-colors'>
                  {dict.auth_forgot_pass}
                </Link>
              </div>
            )}
            ...
            {!isUsernameMode && (
              <div className='text-center mt-6'>
                <span className='text-sm text-gray-400'>
                  {mode === 'login'
                    ? dict.auth_no_account + ' '
                    : dict.auth_has_account + ' '}
                </span>
                <Link
                  href={mode === 'login' ? '/register' : '/login'}
                  className='text-sm text-blue-500 hover:text-blue-400 transition-colors'>
                  {mode === 'login' ? dict.auth_link_register : dict.auth_link_login}
                </Link>
              </div>
            )}
          </form>
